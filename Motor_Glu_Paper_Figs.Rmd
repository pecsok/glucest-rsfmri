...
for (network in networks) {
  ...
  # Exclude tapping outliers if not already done:
  ...
  graph_df$'cest_fc' <- graph_df$avgCEST_SomMot/graph_df$SomMot
  graph_df$'cest_fc' <- scale(graph_df$'cest_fc')
  print(nrow(grp_df))
  print(nrow(graph_df))
...
#print(grp_df)
  ...
  graph_df <- graph_df[!(grp_df$BBLID=="119791"),] # tap_tot>125
