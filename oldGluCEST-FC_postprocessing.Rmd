...
source("/Users/pecsok/GluCESTfMRI/glucest-rsfmri/read_grp_data.R")
...
CESTnetworks = c("avgCEST_SomMot", "avgCEST_DorsAttn",  "avgCEST_Vis","avgCEST_Cont", "avgCEST_Default", "avgCEST_Limbic","avgCEST_SalVentAttn") #  "ctCEST_Cont", "ctCEST_Default",, "ctCEST_Limbic",  ,"ctCEST_SalVentAttn"
...
# Vis GluCEST vs Tapping Score
m1 <- lm(tap_tot ~ avgCEST_Vis + Vis + age + sex, data = grp_df)
gg <- ggplot(grp_df, 
             aes(x = avgCEST_Vis,
                 y = tap_tot)) + #, color=hstatus
  geom_point() + #aes(color=treatment)
 ...
 # theme_pander() +
  theme(legend.position="bottom") +
  stat_fit_glance(label.x = 15, label.y = 120, 
                 mapping = aes(label = paste("R^2 = ", sprintf("%.2f", after_stat(r.squared)), 
                                             "\np = ", sprintf("%.2e", after_stat(p.value)))),
                 show.legend = TRUE) # Ensure the legend is displayed
...
# Calculate the average of each network column
cestavgs <- grp_df[, CESTnetworks]
averages <- colMeans(cestavgs, na.rm = TRUE)
sds <- apply(cestavgs, 2, sd, na.rm = TRUE)

# Create a bar graph of the averages
barplot(averages, main="Average Network Values", xlab="Network", ylab="Average Value",
        names.arg=CESTnetworks, ylim = c(0,16))

# Run pairwise t-tests on each pair of network columns
pairwise_ttests <- lapply(columns_to_test, function(col) {
  pairwise.t.test(df[[col]], df$grouping_variable, p.adjust.method = "none")
})

pairwise_ttests <- pairwise.t.test(df[, CESTnetworks], p.adjust.method = "none")

# Print the results of the pairwise t-tests
print(pairwise_ttests)
